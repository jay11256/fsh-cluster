...
def train_epoch(
    ...
    for cur_iter, (inputs, labels, _vid_idx, meta) in enumerate(train_loader):
        ...
        if cfg.DATA.MULTI_LABEL:
            ...
            few_shotk_correct = metrics.topks_correct(patch_q2s_logits, q2s_labels, (1, 2))
            # num_classes = patch_q2s_logits.size(1)
            # topk_list = tuple(k for k in (1, 5) if k <= num_classes)

            # few_shotk_correct = metrics.topks_correct(
            #     patch_q2s_logits,
            #     q2s_labels,
            #     topk_list
            # )
