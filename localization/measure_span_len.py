...
HERE = "/fs/vulcan-projects/fsh_track/bhargav/sandboxes/fishtal"
sys.path.insert(0, HERE)
from former import FishFormer
from data import list_recordings, load_recording, FG_CLASSES, NUM_CLASSES, STRIDE
from train_former import predict_spans
...
for label, rec in recs_to_test:
    ...
    model = FishFormer(feat_dim=ckpt["feat_dim"], num_classes=NUM_CLASSES, hidden=cfg["hidden"],
                       depth=cfg["depth"], num_heads=cfg["num_heads"], drop=0.0, drop_path=0.0,
                       n_levels=cfg["n_levels"], reg_bins=cfg["reg_bins"],
