...
from train import _nms                                              # noqa: E402
...
def assign_targets(events, win_start, win_len, stride, n_levels, span_s,
                   reg_max=64.0, reference_stride=0.5):
    """Anchor-free assignment of point events to pyramid timesteps.

    A level takes responsibility for an event only if the required regression
    distance is inside that level's range, so short behaviors land on fine levels
    and long ones on coarse levels without hand-tuned anchors.

    `lo`/`hi` below are counted in STEPS, and `reg_max=64.0`'s value was
    implicitly calibrated against patchx's 0.5s stride (level 0's hi=8 steps
    -> 4s of real range, comfortably wider than a 2s span_s window). At a
    finer stride (e.g. dino_raw_8fps's 0.125s), the same step count covers 4x
    less real time -- level 0's range shrinks to ~1s, barely wider than
    span_s/2, and almost nothing qualifies as a positive position (verified:
    n_pos collapsed from ~600/window to ~3.5 in a smoke test before this fix).
    Rescaling reg_max by (reference_stride / stride) keeps every level's range
    constant in SECONDS regardless of feature stride -- a no-op whenever
    stride == reference_stride, i.e. every already-reported patchx-based
    result (patchx/patchx_mil/patchx_motion/patchx_spatial are all 0.5s/step).
    """
    reg_max = reg_max * (reference_stride / stride)
    targets = []
    half = span_s / 2.0
    for lvl in range(n_levels):
        ...
        lo = 0.0 if lvl == 0 else reg_max * (2 ** (lvl - 1)) / 8.0
        hi = reg_max * (2 ** lvl) / 8.0 if lvl < n_levels - 1 else 1e9

        for t_abs, c in events:
            t_rel = t_abs - win_start * stride          # seconds into window
            s, e = t_rel - half, t_rel + half
            idx = np.arange(t_len, dtype=np.float32) * (lvl_stride * stride)
            left = (idx - s) / (lvl_stride * stride)     # distance in level steps
            right = (e - idx) / (lvl_stride * stride)
            inside = (left > 0) & (right > 0)
            fits = inside & (np.maximum(left, right) >= lo) & (np.maximum(left, right) <= hi)
            if not fits.any():
                continue
            cls_t[fits] = c
            ...
            pos |= fits
        targets.append({"cls": cls_t, "reg": reg_t, "ctr": ctr_t, "pos": pos})
    return targets
...
class FormerWindows(Dataset):
    ...
    def __init__(self, recordings, feature_mode, window_s, span_s, n_levels,
        ...
        self.span_s, self.n_levels = span_s, n_levels
        self.train = train
    ...
    def __getitem__(self, i):
        ...
        tg = assign_targets(ev, start, self.win, self.stride, self.n_levels, self.span_s)
